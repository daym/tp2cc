...
template <typename T, auto Lo, std::size_t N>
struct tp2cc_is_array<tp2cc_Array<T, Lo, N>> : std::true_type {};

template <typename T>
inline constexpr bool tp2cc_is_array_v = tp2cc_is_array<T>::value;

template <typename Array>
struct tp2cc_ArrayAddrTraits;

template <typename T, auto Lo, std::size_t N>
struct tp2cc_ArrayAddrTraits<tp2cc_Array<T, Lo, N>> {
  using array_type = tp2cc_Array<T, Lo, N>;
  using element_type = T;
};

template <typename Array>
class tp2cc_ArrayAddr {
 public:
  using raw_array_type = std::remove_cv_t<Array>;
  using traits = tp2cc_ArrayAddrTraits<raw_array_type>;
  using array_type = std::conditional_t<std::is_const_v<Array>,
                                        const typename traits::array_type,
                                        typename traits::array_type>;
  using element_type = std::conditional_t<std::is_const_v<Array>,
                                          const typename traits::element_type,
                                          typename traits::element_type>;

  constexpr explicit tp2cc_ArrayAddr(array_type* p) : p_(p) {}

  // FPC accepts the same raw `@arr` designator in both pointer-to-array and
  // pointer-to-first-element contexts. Preserve that source-level ambiguity
  // until C++ overload resolution / assignment context chooses.
  constexpr operator array_type*() const { return p_; }
  constexpr operator element_type*() const {
    // `tp2cc_Array` deliberately stores the Pascal payload as its sole data
    // member at offset zero, so the array object's address is also the
    // address of element zero. Spell the conversion through reinterpret_cast
    // instead of `p_->data` so nil-safe synthetic addresses like
    // `@p^.field` never dereference the base pointer.
    return reinterpret_cast<element_type*>(p_);
  }
  template <typename E = element_type,
            typename = std::enable_if_t<
                sizeof(std::remove_cv_t<E>) == 1 &&
                !std::is_same_v<std::remove_cv_t<E>, p_char>>>
  constexpr operator std::conditional_t<std::is_const_v<E>, const p_char*,
                                        p_char*>() const {
    using out_ptr =
        std::conditional_t<std::is_const_v<E>, const p_char*, p_char*>;
    return reinterpret_cast<out_ptr>(p_);
  }
  template <typename A = Array,
            typename = std::enable_if_t<!std::is_const_v<A>>>
...
template <typename Array,
          typename = std::enable_if_t<tp2cc_is_array_v<std::remove_cv_t<Array>>>>
constexpr tp2cc_ArrayAddr<Array> tp2cc_array_addr(Array& array) {
  return tp2cc_ArrayAddr<Array>(std::addressof(array));
}

template <typename Array,
          typename = std::enable_if_t<tp2cc_is_array_v<std::remove_cv_t<Array>>>>
constexpr tp2cc_ArrayAddr<Array> tp2cc_array_addr(Array* array) {
  return tp2cc_ArrayAddr<Array>(array);
}
